...
";
        ...
        private void SetCaptions()
        {
            ...
            colAddressName.Text = oDict.GetWord("address_name");
        ...
        private void BindClientList()
        {
            ...
            while (reader.Read())
            {
                ListViewItem objItem = this.lvwClientList.Items.Add(reader.GetString(3));  // Address Name
                #region Icon
                bool primaryRec = reader.GetBoolean(2);
                if (primaryRec)
                {
                    objItem.SmallImage = new IconResourceHandle("16x16.vcard_key_16.png");
                    objItem.LargeImage = new IconResourceHandle("Icons.32x32.vcard_key_32.png");
                }
                else
                {
                    objItem.SmallImage = new IconResourceHandle("16x16.vcard.png");
                    objItem.LargeImage = new IconResourceHandle("Icons.32x32.vcard_32.png");
                }
                #endregion
                objItem.SubItems.Add(reader.GetInt32(0).ToString());  // ID
                objItem.SubItems.Add(iCount.ToString());    // Line Number
                objItem.SubItems.Add(reader.GetString(4));  // Address
                objItem.SubItems.Add(reader.GetString(5));  // Tel
                objItem.SubItems.Add(reader.GetString(6));  // Fax
                objItem.SubItems.Add(reader.GetString(7));  // Contact Person
                objItem.SubItems.Add(reader.GetString(8));  // Mobile
...
//            _CurShortcut = String.Empty;
        ...
        private void DoLookup()
        {
            ...
            if (!(String.IsNullOrEmpty(target)))
            {
                ...
                _CurSqlWhere = _BaseSqlWhere + String.Format(" AND ") +
                               String.Format(" ([Name] LIKE N'%{0}%' OR [Address] LIKE N'%{0}%' OR [ContactPerson] LIKE N'%{0}%')", target);
                BindClientList();
                this.Update();
            }
        }
        ...
        private void ansWorkshop_MenuClick(object sender, MenuItemEventArgs e)
        {
            ...
            _CurWorkshop = (string)e.MenuItem.Tag;
            BindClientList();
            this.Update();
        }
