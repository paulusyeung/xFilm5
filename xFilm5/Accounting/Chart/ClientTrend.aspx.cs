...
{
    public partial class ClientTrend : System.Web.UI.Page
    {
...
FROM [dbo].[vwClient_JoiningTrend]
WHERE [Year] >= 2010
ORDER BY [BranchName]
";
...
";
        ...
        private void ChartByBranch()
        {
            ...
            foreach (int year in _Year)
            {
                ...
                series.DataFilters.Add("BranchName", "System.String", DataFilterCondition.Equal, cboFilter.SelectedItem.Text);
                series.DataFilters.Add("Year", "System.Int32", DataFilterCondition.Equal, year);
                series.ValueDataMembers.AddRange(new string [] {"BranchYTD"});
            ...
            ((XYDiagram)wccRevenue.Diagram).AxisY.Title.Text = "Counts";
        }
        ...
        private void ChartByYear()
        {
            ...
            foreach (string branch in _Branch)
            {
                ...
                series.ArgumentDataMember = "Month";
                series.DataFilters.Add("BranchName", "System.String", DataFilterCondition.Equal, branch);
        ...
        protected void cmdRefresh_ButtonClick(object source, DevExpress.Web.ButtonEditClickEventArgs e)
        {
            ...
            SqlDs.SelectCommand = @"SELECT [BranchName]
      ,[Year]
      ,[Month]
      ,SUM([Count]) AS CurMonth
      ,(
      SELECT COUNT(*)
      FROM [dbo].[vwClient_JoiningTrend] b
      WHERE a.BranchName = b.BranchName
		AND (a.Year * 12 + a.Month) >= (b.Year * 12 + b.Month)
      ) AS BranchYTD
FROM [dbo].[vwClient_JoiningTrend] a
GROUP BY [BranchName], [Year], [Month]
ORDER BY [BranchName], [Year], [Month]
";
            wccRevenue.DataBind();
        }
    }
