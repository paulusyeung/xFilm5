...
--      ,[ReceiptDetailId]
      ,[ItemCode]
      ,[ItemDescription]
      ,SUBSTRING([ItemDescription], 10, len([ItemDescription]) - CHARINDEX('.', reverse([ItemDescription])) + 1 - 10) AS FileBaseName
      ,[ItemQty]
      ,[ItemUoM]
      ,[ItemUnitAmt]
      ,[ItemDiscount]
...
", ClientId.ToString(), year.ToString(), month.ToString());
        ...
        public static DataTable Receipts_NotInvoiced(int ClientId, int year, int month)
        {
            string sql = String.Format(@"
SELECT CASE WHEN Ln = 1 THEN CAST([idx] AS VARCHAR(10)) ELSE '' END AS idx
      ,CASE WHEN Ln = 1 THEN CAST([ClientId] AS VARCHAR(10)) ELSE '' END AS ClientId
...
SELECT DENSE_RANK() OVER(Order By [ReceiptHeaderId]) AS idx
      ...
      ,[IsBilled]
FROM [dbo].[vwReceiptDetailsList_Ex]
WHERE ([ClientId] = {0}) AND (YEAR([ReceiptDate]) = {1}) AND (MONTH([ReceiptDate]) = {2}) AND (INMasterId IS NULL)
) AS s
ORDER By [ReceiptHeaderId], [ItemDescription]
", ClientId.ToString(), year.ToString(), month.ToString());
            DataSet dataset = new DataSet();
...
SELECT [InvoiceNumber], [InvoiceDate], [InvoiceAmount]
	  ,CASE WHEN Ln = 1 THEN CAST([idx] AS VARCHAR(10)) ELSE '' END AS idx
      ,CASE WHEN Ln = 1 THEN CAST([ClientId] AS VARCHAR(10)) ELSE '' END AS ClientId
      ,CASE WHEN Ln = 1 THEN CAST([ClientName] AS VARCHAR(10)) ELSE '' END AS ClientName
--      ,[ClientAddress]
--      ,[ClientTel]
...
SELECT DENSE_RANK() OVER(Order By [ReceiptHeaderId]) AS idx
      ...
      ,[OrderPkPrintQueueVpsId]
      ,[OrderHeaderId]
FROM [dbo].[vwReceiptDetailsList_Invoice]
